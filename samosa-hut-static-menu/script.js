...
function setupOptionalImages(root = document) {
  root
    ...
    .forEach((container) => {
      const image =
        container.querySelector("[data-image]");
...
async function chooseSearchMatch(
  index
) {
  const match =
    state.searchMatches[index];
...
async function loadMenu() {
  try {
    ...
    elements.pageStage.innerHTML = `
      <p class="error-state">
        The menu could not load. If you
        opened index.html directly, run
        it through GitHub Pages,
        Cloudflare Pages or a local web
        server so menu.json can be read.
      </p>
    `;
