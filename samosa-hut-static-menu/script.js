...
const elements = {
  ...
  categoriesButton: document.querySelector(
    "#categories-button",
  ),
  nextButton: document.querySelector("#next-button"),
  searchTrigger: document.querySelector(
    "#search-trigger",
  ),
  searchOverlay: document.querySelector(
    "#search-overlay",
  ),
...
function itemAnchor(categoryId, itemName) {
  return `item-${slugify(categoryId)}-${slugify(
    itemName,
  )}`;
}
  ...
  if (item.image) {
    ...
    return `
      <button
        ...
        aria-label="Enlarge image of ${escapeHTML(
          item.name,
        )}"
      >
        <img
...
function renderItem(category, item, index) {
  ...
  const isToday =
    isTodaySpecial(category, item);

  return `
    <article
      id="${escapeHTML(
        itemAnchor(category.id, item.name),
      )}"
      class="menu-item${
        hasImage ? " has-image" : ""
      }${
    ...
    >
      ...
      <div class="menu-item-content">
        ${
          isToday
            ? `
              <span class="today-special-label">
                Today’s Special
              </span>
            `
            : ""
...
function setupSwipeHint() {
  ...
  hint.innerHTML = `
    ...
    <span class="swipe-hint-copy">
      ...
      <small>
        Or tap any category you would like
      </small>
    </span>
  `;
...
function bindEvents() {
  ...
  elements.pageStage.addEventListener(
    ...
    (event) => {
      ...
      /*
        On the Categories page, a swipe
      ...
      */
      ...
      /*
        Keep the normal swipe behaviour
        on all menu category pages.
      */
      goTo(
        state.currentIndex +
          (dx < 0 ? 1 : -1),
...
async function loadMenu() {
  try {
    ...
    elements.pageStage.innerHTML = `
      <p class="error-state">
        The menu could not load. Make sure
        menu.json is uploaded beside index.html.
      </p>
    `;
